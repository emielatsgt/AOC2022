...
class Directory {
    ...
    var size: Int {
        subdirectories.reduce(0) { $0 + $1.size } +
        files.reduce(0) { $0 + $1.size }
    }
    
    @discardableResult
    func mkdir(name: String) -> Directory {
        if let existing = subdirectories.first(where: { $0.name == name }) { return existing }
        
        let newDirectory = Directory(name: name)
        newDirectory.parent = self
        subdirectories.append(newDirectory)
        return newDirectory
    }
}

func parse(_ lines: [String]) -> Directory {
    let rootNode = Directory(name: "/")
